...
import { Card, Collapse, Col, Button, Dropdown } from 'react-bootstrap';
import CardDropdown from 'components/common/CardDropdown';
import { Link } from 'react-router-dom';
import Flex from 'components/common/Flex';
import Avatar from 'components/common/Avatar';
import AdvanceTableWrapper from 'components/common/advance-table/AdvanceTableWrapper';
import ApplicationsTableHeader from './TablesHeaderrs/ApplicationsTableHeader';
...
const columns = [
  {
    ...
    Cell: rowData => {
      ...
      return (
        <Link to={`/application/details/${ID}-casper`}>
          <h5 className="mb-0 fs--1">Application {ID}</h5>
        </Link>
      );
    }
  },
  {
    ...
    Cell: rowData => {
      const { name, avatar, ID } = rowData.row.original.candidate;
      return (
  ...
  },
  {
    ...
    Cell: rowData => {
      const { name, avatar, ID } = rowData.row.original.company;
      return (
        <Link to={`/candidate/user-details/${ID}-${name}`}>
          <Flex alignItems="center">
            {avatar.img ? (
...
const SApplications = () => {
  ...
  function load_init(query) {
    SAGetMyOptions('applications', query ? JSON.stringify(query) : null).then(
      res => {
        if (res) {
          res = res.data;
  ...
  return (
    <>
      ...
      >
        <Card className="mb-3">
          <Card.Header>
            <ApplicationsTableHeader totalItems={customers.length} table />
          </Card.Header>
          <Card.Body className="p-0">
            {customers.length === 0 ? (
              <div>
                <Emptyser message={'Aucune Application'} />
              </div>
            ) : (
              <AdvanceTable
                table
