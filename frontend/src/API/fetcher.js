...
export async function GetOffers(level, query) {
  let url = baseUrl + '/djondoapi/royal/offers/getall';
  return await axios
    .request({
      baseURL: url,
      method: 'get',
...
export async function GetOffer(ID, level, userId) {
  let url = baseUrl + '/djondoapi/royal/offers/getunity/' + ID;
...
export async function ApplyToOffer(data, ID, action) {
  let url = baseUrl + '/djondoapi/royal/offers/applyto/' + ID;
...
export async function ApplyToUser(data, ID, action) {
  let url = baseUrl + '/djondoapi/royal/offers/applyto/candidate/' + ID;
  url += action ? '?action=' + action : '';
...
export async function RespondToDemand(data, ID, action) {
  let url = baseUrl + '/djondoapi/royal/offers/respondto/candidate/' + ID;
  url += action ? '?action=' + action : '';
  return await axios.request({
    baseURL: url,
    method: 'post',
...
export async function RespondToApplication(data, ID, action) {
  let url = baseUrl + '/djondoapi/royal/offers/respondto/offer/' + ID;
  url += action ? '?action=' + action : '';
  return await axios.request({
    baseURL: url,
    method: 'post',
...
export async function SearchText(data, src) {
  let url = baseUrl + '/djondoapi/royal/search/term?term=' + data + '&src=' + src;
